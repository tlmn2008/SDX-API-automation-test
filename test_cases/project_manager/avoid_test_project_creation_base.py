...
class TestCreateProject(base.Base):
    ...
    def test_SKTE_T2495_create_and_delete_empty_project(self):
        uuid = send_project_m_req.create_project(self.token, project_name='auto test project')
    ...
    def test_SKTE_T2624_create_start_stop_delete_task_(self):
        ...
        # 创建项目
        project_id = send_project_m_req.create_project(self.token, project_name='auto test project 4 task')
        # 创建任务
        task_uuid = send_project_m_req.create_project_task(self.token, task_name='automation task', project_id=project_id,
